...
csvw::csvw(const std::string& pcfFileName, const bool& pcfWriteDoc, const string& pcfChildDoc)
{
    ...
    if (!iFile.is_open())
    {
        cout << color::set(color::RED) << "CSV file output failed. CSV file \'" << pcfFileName << "\' unable to be written to." << color::clear() << endl;
        if (pcfWriteDoc)
        {
            ...
            iFile << "#     (2) rows beginning with \'#\' -- these act as comments\n";
