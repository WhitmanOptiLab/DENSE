#include "baby_cl.hpp"
#include "determ.hpp"
#include "simpson.hpp"
#include "trap.hpp"
#include "avg.hpp"
#include <iostream>

/*
template<class Numerical_Simulation>
dense::baby_cl::baby_cl(Numerical_Simulation& sim)
: _width{sim.cell_count()} {
    unsigned sum = 0;
    for (int i = 0; i < NUM_SPECIES; i++) {
        //_position[i] = sum * _width; //for each specie
        _specie_size[i] = (sim.max_delays[i] / sim._step_size) + sim._num_history_steps;
        sum += _specie_size[i];
    }
    _total_length = sum * _width;
    _array = decltype(_array){new Real[_total_length + sim.num_growth_cells()]()};
}

dense::baby_cl::baby_cl(Simpson_Simulation& sim)
: _width{sim.cell_count()} {
    unsigned sum = 0;
    for (int i = 0; i < NUM_SPECIES; i++) {
        //_position[i] = sum * _width; //for each specie
        _specie_size[i] = (sim.max_delays[i] / sim._step_size) + sim._num_history_steps;
        sum += _specie_size[i];
    }
    _total_length = sum * _width;
    _array = decltype(_array){new Real[_total_length]()};
}

dense::baby_cl::baby_cl(Trapezoid_Simulation& sim)
: _width{sim.cell_count()} {
    unsigned sum = 0;
    for (int i = 0; i < NUM_SPECIES; i++) {
        //_position[i] = sum * _width; //for each specie
        _specie_size[i] = (sim.max_delays[i] / sim._step_size) + sim._num_history_steps;
        sum += _specie_size[i];
    }
    _total_length = sum * _width;
    _array = decltype(_array){new Real[_total_length]()};
}

dense::baby_cl::baby_cl(Average_Simulation& sim)
: _width{sim.cell_count()} {
    unsigned sum = 0;
    for (int i = 0; i < NUM_SPECIES; i++) {
        //_position[i] = sum * _width; //for each specie
        _specie_size[i] = (sim.max_delays[i] / sim._step_size) + sim._num_history_steps;
        sum += _specie_size[i];
    }
    _total_length = sum * _width;
    _array = decltype(_array){new Real[_total_length]()};
}
*/
