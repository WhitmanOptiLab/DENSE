...
template<>
template<class Ctxt>
RATETYPE reaction<convergence_synthesis>::active_rate(const Ctxt& c) {
    return c.getRate(convergence_synthesis);
}

template<>
template<class Ctxt>
RATETYPE reaction<convergence_degredation>::active_rate(const Ctxt& c) {
    return c.getRate(convergence_degredation) * c.getCon(convergence_specie);
}
