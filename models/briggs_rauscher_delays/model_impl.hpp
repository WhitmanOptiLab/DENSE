...
template<>
template<class Ctxt>
RATETYPE reaction<i_decay_delay>::active_rate(const Ctxt& c) {
    return c.getRate(i_decay_delay)*(c.getCon(I), c.getDelay(dreact_i_decay_delay));
}

template<>
template<class Ctxt>
RATETYPE reaction<hio_decay_delay>::active_rate(const Ctxt& c) {
    return c.getRate(hio_decay_delay)*(c.getCon(HIO), c.getDelay(dreact_hio_decay_delay));
}
#endif // MODEL_IMPL_H
