...
RATETYPE reaction<mRNA_synthesis>::active_rate(const Ctxt& c) const {
    return c.getRate(mRNA_synthesis);
}
...
template<>
template<class Ctxt>
RATETYPE reaction<mRNA2_synthesis>::active_rate(const Ctxt& c) const {
    return c.getRate(mRNA2_synthesis) * c.calculateNeighborAvg(protein,c.getDelay(dreact_mRNA2_synthesis));
}
