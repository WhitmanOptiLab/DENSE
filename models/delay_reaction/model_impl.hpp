...
template<>
template<class Ctxt>
RATETYPE reaction<reac1>::active_rate(const Ctxt& c) {
    return c.getRate(reac1)+c.getCon(specie1,c.getDelay(dreact_reac1));
}

template<>
template<class Ctxt>
RATETYPE reaction<reac2>::active_rate(const Ctxt& c) {
    return c.getRate(reac2)+c.getCon(specie2,c.getDelay(dreact_reac1));
}
